...
from geometry_msgs.msg import PoseStamped, Point, Twist  # Dodano Twist
...
class ArmController(Node):
    def __init__(self):
        ...
        self.get_logger().info("Arm Controller Node started (With Waist Control).")

        self.declare_parameter("use_robot", True)
        self.declare_parameter("interface", "eno1") # Zmieniono domyślny na eno1, zgodnie z Twoim kodem
        ...
        # --- WAIST CONTROL VARIABLES ---
        self.waist_yaw_index = 12  # G1 WaistYaw Joint Index
        self.waist_yaw_target_pos = 0.0
        self.waist_yaw_target_vel = 0.0
        self.waist_kp = 60.0
        self.waist_kd = 2.0
        self.waist_limit_rad = 1.0
        self._waist_synced = False  # Flaga synchronizacji startowej
        ...
        if not self.use_robot:
            ...
            self._waist_synced = True # W symulacji uznajemy za zsynchornizowany
    ...
    def _hold_non_arm_joints(self):
        """
        Utrzymuje pozycję stawów innych niż ramiona.
        ZMODYFIKOWANE: Tułów (WaistYaw) jest sterowany aktywnie (zadana pozycja),
        a nie tylko "zamrożony" na aktualnej pozycji.
        """
        if not self.use_robot:
            return
        ...
        for jid in G1_29_JointIndex:
            ...
            # --- SEKCJA MODYFIKACJI TUŁOWIA ---
            if jid.value == self.waist_yaw_index and self._waist_synced:
                # Tułów sterujemy aktywnie nawet w trybie hold
                self.msg.motor_cmd[jid].mode = 1
    ...
    def _update_waist_target(self, dt):
        """Aktualizuje cel dla tułowia i zapewnia synchronizację na starcie."""
        if self.use_robot:
            # Synchronizacja na starcie: jeśli nie zsynchro, pobierz aktualną pozycję robota
            if not self._waist_synced:
                robot_data = self.lowstate_buffer.GetData()
                if robot_data is not None:
                    self.waist_yaw_target_pos = float(robot_data.motor_state[self.waist_yaw_index].q)
                    self._waist_synced = True
                    self.get_logger().info(f"Waist synchronized to: {self.waist_yaw_target_pos:.3f} rad")
    ...
    def main_loop(self):
        ...
        # --- ZAWSZE OBLICZAJ FIZYKĘ TUŁOWIA ---
        dt = self._compute_dt()
        self._update_waist_target(dt)
        ...
        # JEŚLI RAMIONA SĄ WYŁĄCZONE (ale tułów chcemy sterować)
        if not self.arms_enabled:
            # Teraz ta funkcja obsłuży poprawnie i trzymanie nóg i ruch tułowia
            self._hold_non_arm_joints()
            return

        # DALSZA CZĘŚĆ TO LOGIKA RAMION (ARMS ENABLED)
        
        # Logika Homing (powrót do pozycji startowej)
        ...
        if self.use_robot:
            ...
            # 1. Przypisz komendy dla RAMION (z IK)
            wrist_vals = {m.value for m in G1_29_JointWristIndex}
            ...
            # 2. Przypisz komendy dla TUŁOWIA (Explicit Waist Control)
            # Ważne: Nawet jak ruszamy rękami, tułów musi dostawać swoją komendę!
            if self._waist_synced:
                 for jid in G1_29_JointIndex:
