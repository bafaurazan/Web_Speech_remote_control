...
ros2 topic pub -1 /g1pilot/hand_goal/left geometry_msgs/msg/PoseStamped "{header: {stamp: {sec: 0, nanosec: 0}, frame_id: 'pelvis'}, pose: {position: {x: 0.50, y: 0.17, z: 0.09}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0}}}"
```
...
# note
```md
for simulation mode type publish_joint_states:=false (for robot_state node) and use_robot:=false (for rest nodes) 

for real robot mode type publish_joint_states:=true (for robot_state node) and use_robot:=true (for rest nodes) 
```