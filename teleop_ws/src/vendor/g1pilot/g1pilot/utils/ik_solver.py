...
class G1IKSolver:
    ...
    def __init__(self,
        ...
        # Postural regularization – lekkie przyciąganie do poprzedniej konfiguracji
        # (oddzielnie dla lewej i prawej ręki), żeby unikać gwałtownych "flipów"
        # łokcia/ramienia dla tego samego położenia dłoni.
        ...
        self._prev_q14 = None
        self._prev_arm_q = {"left": None, "right": None}
    ...
    def solve(self, side: str, q_init: np.ndarray, current_all: np.ndarray) -> np.ndarray:
        ...
        # Poprzednia konfiguracja tej ręki (do postural regularization)
        if self._prev_arm_q[side] is None:
            # Startujemy z aktualnej pozycji jako "referencji"
            self._prev_arm_q[side] = np.array(
                [float(q[self._name_to_q_index[self._ros_joint_names[i]]]) for i in arm_ids],
                dtype=float,
            )
        ...
        for _ in range(self.max_iter):
            ...
            # Postural regularization: lekkie przyciąganie do poprzedniej konfiguracji
            if self.posture_gain > 0.0 and self._prev_arm_q[side] is not None:
                q_arm_cur = np.array(
                    [float(q[self._name_to_q_index[self._ros_joint_names[i]]]) for i in arm_ids],
                    dtype=float,
                )
                dq_post = -self.posture_gain * (q_arm_cur - self._prev_arm_q[side])
                dq_red += dq_post
        ...
        # Zapisz jako referencję dla kolejnego wywołania (postural regularization)
        self._prev_arm_q[side] = q_arm.copy()
        return q_arm
